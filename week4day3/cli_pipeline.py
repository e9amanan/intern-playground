...
try:
    ...
    class DataValidator:
        ...
        def validate_task_csv(self, path):
            return [
                {"title": "Setup Django", "description": "Initial config", "status": "done", "categories": ["Python", "Backend"], "due_date": "2026-08-01"},
                {"title": "Database Models", "description": "Write ORM classes", "status": "in_progress", "categories": ["Django", "SQL"], "due_date": "2026-08-05"}
            ]
    def setup_database(db_name):
        with sqlite3.connect(db_name) as conn:
            conn.execute("CREATE TABLE IF NOT EXISTS categories (id INTEGER PRIMARY KEY, name TEXT UNIQUE);")
            conn.execute("CREATE TABLE IF NOT EXISTS tasks (id INTEGER PRIMARY KEY, title TEXT, description TEXT, status TEXT, category_count INTEGER, is_overdue INTEGER);")
            conn.execute("CREATE TABLE IF NOT EXISTS tasks_categories (task_id INTEGER, category_id INTEGER, PRIMARY KEY(task_id, category_id));")
...
def run_validate(args: argparse.Namespace, config: Dict[str, Any], logger: logging.Logger) -> int:
    ...
    if validator.errors:
        logger.warning("Validation completed with %d schema errors", len(validator.errors))
        for err in validator.errors:
            logger.warning("Row %s | Field '%s': %s", err.get("row"), err.get("field"), err.get("error"))
        if args.strict:
...
def run_transform(args: argparse.Namespace, config: Dict[str, Any], logger: logging.Logger) -> int:
    ...
    logger.debug("Enriching records with derived fields (category_count, is_overdue)...")
    for task in clean_data:
        ...
        if task.get('due_date'):
            try:
                ...
                logger.debug("Invalid date format for due_date: %s", task.get('due_date'))
    ...
    logger.info("Transformation complete. Wrote %d records to %s", len(clean_data), out_file)
    return 0


def run_load(args: argparse.Namespace, config: Dict[str, Any], logger: logging.Logger) -> int:
    ...
    try:
        with sqlite3.connect(db_name) as conn:
            ...
            for task in clean_data:
                cursor.execute('''
                    ...
                    VALUES (?, ?, ?, ?, ?)
                ''', (task['title'], task['description'], task['status'], task['category_count'], int(task['is_overdue'])))
        ...
        logger.info("Successfully loaded data into SQLite database without constraint violations.")
        return 0
    except Exception as exc:
        logger.critical("CRITICAL ERROR during LOAD: %s", exc, exc_info=args.verbose >= 2)
        logger.critical("Entire database transaction has been automatically rolled back.")
        return 1


def run_report(args: argparse.Namespace, config: Dict[str, Any], logger: logging.Logger) -> int:
...
def build_parser() -> argparse.ArgumentParser:
    ...
    val_parser.add_argument("file", type=validate_file_exists, help="Path to input CSV file.")
    val_parser.add_argument("--strict", action="store_true", help="Abort pipeline on schema warnings.")
    val_parser.set_defaults(func=run_validate)

    # --- Subcommand: transform ---
    trans_parser = subparsers.add_parser("transform", help="Enrich dataset with derived fields.")
    trans_parser.add_argument("file", type=validate_file_exists, help="Path to input CSV file.")
    trans_parser.set_defaults(func=run_transform)

    # --- Subcommand: load ---
    load_parser = subparsers.add_parser("load", help="Load transformed data into SQLite.")
    load_parser.add_argument("file", type=validate_file_exists, help="Path to input CSV file.")
    load_parser.add_argument("--db", type=str, help="Target SQLite database filename override.")
    load_parser.set_defaults(func=run_load)

    # --- Subcommand: report ---
    rep_parser = subparsers.add_parser("report", help="Generate statistics report from DB.")
    rep_parser.add_argument("--db", type=str, help="Target SQLite database filename to report on.")
...
def main() -> int:
    ...
    try:
        ...
        logger.critical("Fatal unhandled pipeline exception: %s", exc, exc_info=args.verbose >= 2)
        return 1
