...
def section_3_constraints_and_relationships():
    ...
    with sqlite3.connect(DB_NAME) as conn:
        conn.execute("PRAGMA foreign_keys = 1") # MUST turn this on in SQLite to enforce FKs
        cursor = conn.cursor()
        ...
        # Insert Valid Data
        cursor.execute("INSERT INTO authors (email, name) VALUES ('writer@test.com', 'Stephen')")
        author_id = cursor.lastrowid
        cursor.execute("INSERT INTO books (title, author_id) VALUES ('The Shining', ?)", (author_id,))
        
        # Constraint Test 1: UNIQUE constraint failure
        try:
            cursor.execute("INSERT INTO authors (email, name) VALUES ('writer@test.com', 'King')")
        except sqlite3.IntegrityError as e:
            print(f"Caught Expected UNIQUE Error: {e}")
            
        # Constraint Test 2: CHECK constraint failure
        try:
            cursor.execute("INSERT INTO books (title, author_id, status) VALUES ('IT', ?, 'invalid')'", (author_id,))
        except sqlite3.IntegrityError as e:
            print(f"Caught Expected CHECK Error: {e}")
            
        # Constraint Test 3: FOREIGN KEY constraint failure (Author ID 999 doesn't exist)
        try:
            cursor.execute("INSERT INTO books (title, author_id) VALUES ('Misery', 999)")
        except sqlite3.IntegrityError as e:
...
def section_4_indexes():
    ...
    with sqlite3.connect(DB_NAME) as conn:
        ...
        cursor.execute("CREATE TABLE products (id INTEGER PRIMARY KEY, sku TEXT, price REAL)")
        cursor.execute("INSERT INTO products (sku, price) VALUES ('ABC-123', 19.99)")
        
        # 1. EXPLAIN QUERY PLAN BEFORE Index
        cursor.execute("EXPLAIN QUERY PLAN SELECT * FROM products WHERE sku = 'ABC-123'")
...
def section_5_transactions_and_sqlite3():
    ...
    cursor.execute("CREATE TABLE bank_accounts (name TEXT, balance INTEGER CHECK(balance >= 0))")
    ...
    try:
        ...
        cursor.execute("UPDATE bank_accounts SET balance = balance - 200 WHERE name = 'Alice'")
        
        # Step 2: Add to Bob (This code won't run because Step 1 throws an error)
        cursor.execute("UPDATE bank_accounts SET balance = balance + 200 WHERE name = 'Bob'")
