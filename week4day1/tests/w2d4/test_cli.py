...
from week2day4.cli_project.yourpkg.cli import main, handle_analyze
...
def test_handle_analyze_file_not_found(mock_file):
    ...
    with pytest.raises(SystemExit, match="File Error: The file 'missing.csv' was not found."):
        handle_analyze(DummyArgs())


@patch("builtins.open", new_callable=mock_open, read_data="timestamp,wrong_column\n2023-10-01,50.0")
def test_handle_analyze_validation_error(mock_file):
    ...
    with pytest.raises(SystemExit, match="Data Validation Error: No valid data rows found"):
        handle_analyze(DummyArgs())